...
function displayRecipeDetails(recipe) {
  document.getElementById("recipeDetails").innerHTML = `
        ...
        <button id="editRecipe" onclick="editRecipe('${
          recipe._id
        }')">Edit</button>
        <button id="deleteRecipe" onclick="deleteRecipe('${
          recipe._id
        }')">Delete</button>`;
}
...
function editRecipe(id) {
  ...
  const ingredient = recipeDetails
    .querySelector("p:nth-child(2)")
    .textContent.replace("Ingredients: ", "");
  const instruction = recipeDetails
    .querySelector("p:nth-child(3)")
    .textContent.replace("Instructions: ", "");
  const mealType = recipeDetails
    .querySelector("p:nth-child(4)")
    .textContent.replace("Meal Type: ", "");
